...
type CreateCustomerCategoryInput struct {
	Name    string `json:"name"`
	Name2   string `json:"name2"`
	OrderNo uint   `json:"orderNo"`
}

func CustomerCategoryList(c *gin.Context) {
...
func CustomerCategoryCreate(c *gin.Context) {
...
func CustomerCategoryGet(c *gin.Context) {
	CCId, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Failed in parse params"})
		return
