...
type CreateRelTypeInput struct {
	Name    string `json:"name"`
	Name2   string `json:"name2"`
	OrderNo uint   `json:"orderNo"`
}

func RelTypeList(c *gin.Context) {
	...
	response := helper_core.CH.Request(helper_core.RELATION_TYPE_LIST, bodyData)
	if response.Err != nil {
...
func RelTypeCreate(c *gin.Context) {
	var input CreateRelTypeInput
	...
	response := helper_core.CH.Request(helper_core.RELATION_TYPE_INSERT, bodyData)
	if response.Err != nil {
...
func RelTypeDelete(c *gin.Context) {
	relTypeID, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Failed in parse params"})
		return
	}
	var bodyData = []byte(fmt.Sprintf(`[{}, %v]`, relTypeID))
	response := helper_core.CH.Request(helper_core.RELATION_TYPE_DELETE, bodyData)
...
func RelTypeGet(c *gin.Context) {
	...
	var bodyData = []byte(fmt.Sprintf(`[%v]`, relTypeID))
	response := helper_core.CH.Request(helper_core.RELATION_TYPE_SELECT, bodyData)
